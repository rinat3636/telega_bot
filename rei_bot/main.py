...
async def main():
    ...
    # Регистрация роутеров ВСЕГДА (независимо от ENABLE_*)
    # Проверка ENABLE_* выполняется ВНУТРИ handlers
    # Это соответствует спецификации (пункт 5): каждая кнопка обязана иметь handler
    dp.include_router(common.router)
    dp.include_router(admin.router)
    dp.include_router(balance.router)   # ВСЕГДА
    dp.include_router(videos.router)    # ВСЕГДА
    dp.include_router(images.router)    # ВСЕГДА (последним, т.к. обрабатывает текст)
    dp.include_router(global_commands.router)  # fallback в конце
    
    # Логирование состояния функций
