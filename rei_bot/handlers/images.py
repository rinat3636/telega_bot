...
async def create_image_button(message: Message, state: FSMContext):
    """
    Обработчик кнопки "🎨 Создать изображение"
    Согласно спецификации (пункт 5): каждая кнопка обязана иметь handler
    """
    await state.clear()
    
    # Проверка ENABLE_IMAGES
    if not config.ENABLE_IMAGES:
        await message.answer("⚠️ Генерация изображений временно отключена")
        return
    ...
    text = (
        ...
        "✍️ Напишите текстовый запрос для генерации изображения\n\n"
        "Например:\n"
        "• Кот в космосе\n"
        "• Закат над океаном\n"
        "• Футуристический город\n\n"
        "💰 Стоимость: <b>10 кредитов</b>"
    )
    
    await message.answer(text, parse_mode="HTML")


@router.message(F.text == "🛠 Редактировать изображение")
async def edit_image_button(message: Message, state: FSMContext):
    """
    ...
    # Проверка ENABLE_IMAGES
    if not config.ENABLE_IMAGES:
        await message.answer("⚠️ Редактирование изображений временно отключено")
        return
    ...
    text = (
        "🛠 <b>Редактирование изображения</b>\n\n"
        "📸 Пришлите изображение и опишите, что нужно изменить\n\n"
        "Можно:\n"
        "• Отправить фото с подписью\n"
        "• Отправить фото, затем описание\n\n"
        "Например:\n"
        "• Сделать фон синим\n"
